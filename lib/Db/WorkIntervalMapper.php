...
class WorkIntervalMapper extends CompatibleMapper {
    ...
    public function findByName($name) {
        $sql = 'SELECT * FROM `*PREFIX*nextrcp_work_interval` ' .
            'WHERE `name` = ?';
    ...
    public function findAllForWorkItem($workItemId, $limit=null, $offset=null) {
        $sql = 'SELECT * FROM `*PREFIX*nextrcp_work_interval` where `work_item_id` = ?';
        return $this->findEntities($sql, [$workItemId],$limit, $offset);
    }

    public function findLatest($user, $limit = 10, $offset = 0){
        $sql = 'SELECT * FROM `*PREFIX*nextrcp_work_interval` where user_uid = ? order by start desc';
        return $this->findEntities($sql, [$user],$limit, $offset);
    }

    public function findLatestByName($user, $name){
        $sql = 'SELECT * FROM `*PREFIX*nextrcp_work_interval` where user_uid = ? and name = ? order by start desc';
        try {
    ...
    public function findLatestInterval($user, $from, $to, $limit = 5000, $offset = 0){
        ...
        $sql = 'SELECT * FROM `*PREFIX*nextrcp_work_interval` where '.implode(" and ",$filters).' order by start desc';
        return $this->findEntities($sql, $params, $limit, $offset);
    }

    public function findAllRunning($user, $limit = 100, $offset = 0){
        $sql = 'SELECT * FROM `*PREFIX*nextrcp_work_interval` where user_uid = ? and running = 1 order by start desc';
        return $this->findEntities($sql, [$user],$limit, $offset);
    }

    public function stopAllRunning($user, $limit = 100, $offset = 0){
        $sql = 'update wi FROM `*PREFIX*nextrcp_work_interval` wi set wi.running = 0 where user_uid = ? and running = 1';
        return $this->findEntities($sql, [$user],$limit, $offset);
    }

    public function findAllForProject($project_id){
        $sql = 'SELECT * FROM `*PREFIX*nextrcp_work_interval` where project_id = ?';
        return $this->findEntities($sql, [$project_id]);
    }

    public function deleteAllForProject($project_id) {
        $sql = 'delete FROM `*PREFIX*nextrcp_work_interval` ' .
            ' where project_id = ?';
